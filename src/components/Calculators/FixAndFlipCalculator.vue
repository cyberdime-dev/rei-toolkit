...
const totalInvestment = computed(
  () => purchasePrice.value + repairCosts.value + holdingCosts.value + closingCosts.value
);

const sellingCosts = computed(() => (arv.value * sellingCostPercent.value) / 100);

const netProfit = computed(() => arv.value - sellingCosts.value - totalInvestment.value);

const roi = computed(() =>
  totalInvestment.value === 0 ? 0 : (netProfit.value / totalInvestment.value) * 100
);
...
<template>
  <v-card class="pa-4 mobile-card">
    ...
    <v-row dense>
      ...
      <v-col cols="12" sm="6">
        <v-text-field v-model.number="repairCosts" label="Repair Costs" prefix="$" />
      </v-col>
      <v-col cols="12" sm="6">
        <v-text-field v-model.number="arv" label="ARV (After Repair Value)" prefix="$" />
      </v-col>
      <v-col cols="12" sm="6">
        <v-text-field v-model.number="holdingCosts" label="Holding Costs" prefix="$" />
      </v-col>
      <v-col cols="12" sm="6">
