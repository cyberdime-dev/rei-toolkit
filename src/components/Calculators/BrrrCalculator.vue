...
const totalInitialInvestment = computed(() => purchasePrice.value + rehabCosts.value);
...
const monthlyCashFlow = computed(() => {
  const value = monthlyRent.value - monthlyExpenses.value - monthlyMortgage.value;
  return isNaN(value) ? 0 : value;
});

const cocReturn = computed(() => {
  if (!totalInitialInvestment.value) return 0;
  const value = ((monthlyCashFlow.value * 12) / totalInitialInvestment.value) * 100;
  return isNaN(value) ? 0 : value;
});
...
<template>
  <v-card class="pa-4 mobile-card">
    ...
    <v-row dense>
      ...
      /></v-col>
      <v-col cols="12" sm="6"
        ><v-text-field v-model.number="rehabCosts" label="Rehab Costs" prefix="$"
      /></v-col>
      <v-col cols="12" sm="6"
        ><v-text-field v-model.number="arv" label="After Repair Value (ARV)" prefix="$"
      /></v-col>
      <v-col cols="12" sm="6"
        ><v-text-field v-model.number="refiLtv" label="Refinance LTV (%)" suffix="%"
      /></v-col>

      <!-- Rental and Refi -->
      <v-col cols="12" sm="6"
        ><v-text-field v-model.number="monthlyRent" label="Monthly Rent" prefix="$"
      /></v-col>
      <v-col cols="12" sm="6"
        ><v-text-field
